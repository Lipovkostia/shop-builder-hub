...
### ⏳ Этап 5: Интеграция в AdminPanel (СЛЕДУЮЩИЙ ШАГ)
Требуется заменить текущую таблицу в `AdminPanel.tsx` на `ProductsSection`:
- Передать все необходимые props
- Удалить дублирующийся код из AdminPanel
