-- Drop and recreate the function with correct price calculation
DROP FUNCTION IF EXISTS get_catalog_products_public(_access_code text);

CREATE OR REPLACE FUNCTION get_catalog_products_public(_access_code text)
RETURNS TABLE (
  product_id uuid,
  product_name text,
      ...
      WHEN p.buy_price IS NOT NULL AND p.buy_price > 0 THEN
        CASE 
          WHEN COALESCE(cps.markup_type, 'percent') = 'percent' THEN 
            p.buy_price * (1 + COALESCE(cps.markup_value, 0) / 100)
          WHEN cps.markup_type = 'fixed' OR cps.markup_type = 'rubles' THEN 
            p.buy_price + COALESCE(cps.markup_value, 0)
          ELSE p.buy_price
        END
      ELSE 0
