-- Update get_wholesale_products_public to use catalog_category_settings for custom names and sort order
...
AS $$
  ...
  cat_settings AS (
    SELECT ccs.category_id,
           ccs.custom_name,
           ccs.sort_order
    FROM public.catalog_category_settings ccs
    JOIN s ON ccs.catalog_id = s.wholesale_catalog_id
  )
  SELECT
    p.id,
    p.name,
    ...
    -- Get category name: first check catalog-specific custom_name, then original category name
    COALESCE(
      cs_primary.custom_name,
      c_primary.name,
      cs_product.custom_name,
      c_product.name
    ) AS category_name,
  ...
  LEFT JOIN cat_settings cs_primary ON cs_primary.category_id = st.primary_category_id
  LEFT JOIN public.categories c_product ON c_product.id = p.category_id
  LEFT JOIN cat_settings cs_product ON cs_product.category_id = p.category_id
...
GRANT EXECUTE ON FUNCTION public.get_wholesale_products_public(text) TO anon;
GRANT EXECUTE ON FUNCTION public.get_wholesale_products_public(text) TO authenticated;