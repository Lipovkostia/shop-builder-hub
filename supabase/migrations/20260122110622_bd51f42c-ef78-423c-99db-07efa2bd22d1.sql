-- Public RPC to fetch retail products for storefront without exposing sensitive columns like buy_price
...
AS $$
  ...
  SELECT
    ...
    (
      CASE
        WHEN p.price IS NOT NULL AND p.price > 0 THEN p.price::double precision
...
GRANT EXECUTE ON FUNCTION public.get_retail_products_public(text) TO anon, authenticated;