...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === "process_messages") {
      ...
      // 4. Process each chat with unread messages
      for (const chat of avitoChats) {
        try {
          ...
          const systemPrompt = bot.system_prompt || "Ты — помощник продавца на Авито. Отвечай вежливо и помогай с вопросами о товарах.";
          const proSuffix = bot.pro_seller_mode
            ? "\n\nДополнительно: веди себя как профессиональный продавец. Используй техники продаж, задавай уточняющие вопросы, предлагай дополнительные товары."
            : "";
