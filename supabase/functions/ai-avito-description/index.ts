...
ЖЁСТКИЕ ОГРАНИЧЕНИЯ МОДЕРАЦИИ АВИТО (нарушение = блокировка объявления):
- НИКОГДА не вставляй номера телефонов, e-mail, ссылки (http, .ru, .com), упоминания мессенджеров (@username, telegram, whatsapp).
- НЕ используй КАПС из 5+ букв подряд.
- НЕ пиши слова-триггеры: "гарантия 100%", "лучший", "№1", "дешевле всех", "только сегодня" — это блокирует модерация.

ПЛЕЙСХОЛДЕРЫ: Если в инструкции от продавца или в составленных тобой блоках есть метки вида {product_name}, {price}, {city}, {description} — ОСТАВЬ ИХ В ТЕКСТЕ В ТОЧНО ТАКОМ ВИДЕ С ФИГУРНЫМИ СКОБКАМИ. Система сама подставит реальные значения. НЕ раскрывай их.

ФОРМАТИРОВАНИЕ: Авито поддерживает простое текстовое форматирование. Используй:
- Переносы строк (\\n) для разделения абзацев и смысловых блоков
- Эмодзи для визуального выделения ключевых пунктов (📦, ✅, 🚚, 💰 и т.д.) — но не телефон 📞
- Пустые строки (\\n\\n) между абзацами для читаемости
- Структурируй текст: сначала краткое описание товара, затем характеристики/преимущества списком, затем призыв к действию (CTA) в конце

ВАЖНО ПРО ВИДИМОСТЬ: На мобильном Авито обрезает текст через ~150 символов кнопкой «Показать ещё». Самое важное — в первые 150 символов.
${userInstruction}
