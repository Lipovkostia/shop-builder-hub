...
function buildSystemPrompt(bot: any, catalogContext: string, salesStages: any[]): string {
  ...
  const instructionsConfig = bot.instructions_config || {};
  if (instructionsConfig.clarifying_questions_enabled) {
    prompt += `\n\nВАЖНО — РЕЖИМ УТОЧНЯЮЩИХ ВОПРОСОВ:
Когда покупатель запрашивает товар, и в каталоге есть БОЛЬШЕ 6 подходящих позиций с похожим названием:
1. НЕ показывай сразу все товары.
2. Задай 1-3 коротких уточняющих вопроса, чтобы сузить выбор. Например: "Вам нужна нарезка или целым куском?", "Какой вес интересует?", "Для какого случая — на каждый день или для мероприятия?"
3. Предложи варианты ответов кратко, чтобы клиенту было легко выбрать.
4. После уточнения покажи 3-6 наиболее подходящих позиций с тегом [PRODUCTS:...].
Если подходящих позиций 6 или меньше — показывай сразу без уточнений.`;
  }
