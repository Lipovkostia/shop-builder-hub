        ...
        <div className={`flex-1 min-w-0 flex ${showImages ? 'flex-col justify-center gap-0' : 'flex-row items-center gap-2'}`}>
          ...
          <div className={`${showImages ? '' : 'flex-1 min-w-0 overflow-hidden flex flex-col justify-center'}`}>
            ...
            <div className="flex items-center gap-1">
              ...
              >
                <h3 className={`font-medium text-foreground leading-tight whitespace-nowrap ${showImages ? 'text-[14px] pr-6' : 'text-[11px] pr-4'} ${isDescriptionExpanded ? 'text-primary' : ''}`}>
                  {product.name}
                </h3>
                ...
                {hasPortionPrice && portionPrice && (() => {
                  ...
                  return (
                    <button
                      ...
                      className={`relative flex items-center gap-1 rounded border border-border hover:border-primary hover:bg-primary/5 transition-all ${showImages ? 'h-8 px-2.5' : 'h-7 px-2'}`}
                    >
                      {qty > 0 && (
                        <span className={`absolute -top-1 -right-1 bg-primary text-primary-foreground font-bold rounded-full flex items-center justify-center ${showImages ? 'w-4 h-4 text-[11px]' : 'w-3.5 h-3.5 text-[9px]'}`}>
                          {qty}
                        </span>
                      )}
                      <PortionIndicator type="portion" />
                      <span className={`font-medium text-foreground ${showImages ? 'text-[11px]' : 'text-[9px]'}`}>
                ...
                {!hasVariantPrices && (() => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <Plus className={`text-primary ${showImages ? 'w-3.5 h-3.5' : 'w-3 h-3'}`} />
                      <span className={`font-medium text-foreground ${showImages ? 'text-[11px]' : 'text-[9px]'}`}>
...
const CustomerDashboard = () => {
  ...
  const getItemPriceInfo = (product: CatalogProduct | undefined, variantIndex: number, itemPrice: number): { pricePerUnit: string; unitLabel: string } => {
    ...
    // показываем цену за 1 штуку.
    if (unitLabel === 'шт' && unitWeight > 1) {
      const expectedPackPrice = product.price * unitWeight;
      const isPackPrice = Math.abs(itemPrice - expectedPackPrice) <= Math.max(1, expectedPackPrice * 0.01);

      if (isPackPrice) {
        return { pricePerUnit: formatPrice(itemPrice / unitWeight), unitLabel: 'шт' };
      }
    }

    return { pricePerUnit: formatPrice(itemPrice), unitLabel };
  };
