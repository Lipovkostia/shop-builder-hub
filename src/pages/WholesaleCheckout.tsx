...
import { ArrowLeft, ShoppingBag, Building2, User, Phone, Mail, MessageSquare, Loader2, CheckCircle2, Package, AlertTriangle } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Label } from "@/components/ui/label";
import { Textarea } from "@/components/ui/textarea";
import { useWholesaleStore } from "@/hooks/useWholesaleStore";
...
export default function WholesaleCheckout({ subdomain: propSubdomain }: WholesaleCheckoutProps = {}) {
  const params = useParams();
  const subdomain = propSubdomain || params.subdomain;
  const navigate = useNavigate();
  const { store, loading: storeLoading } = useWholesaleStore(subdomain);
  ...
  const minOrderAmount = store?.wholesale_min_order_amount || 0;
  const isMinOrderMet = !minOrderAmount || cartTotal >= minOrderAmount;
  const remainingAmount = minOrderAmount - cartTotal;
  ...
  useEffect(() => {
    ...
    const timeoutId = setTimeout(() => {
      if (!storeLoading && store && cart.length === 0 && !orderSuccess) {
        navigate(`/wholesale/${subdomain}`, { replace: true });
      }
    }, 100);
  ...
  const isFormValid = contactName.trim() && phone.length >= 10 && isMinOrderMet;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke("create-wholesale-order", {
        body: {
          storeId: store.id,
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-muted-foreground">Оптовый магазин не активирован</p>
        </div>
      </div>
    );
  }
          ...
          <div className="max-w-md mx-auto text-center">
            ...
            <div className="bg-muted/50 rounded-xl p-4 mb-8 text-left">
              ...
              <ul className="text-sm text-muted-foreground space-y-1.5">
                ...
                <li>• Обсудим условия доставки и оплаты</li>
            ...
            <div className="flex items-start gap-3 p-4 rounded-xl bg-amber-500/10 border border-amber-500/30">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-1">
                  Добавьте ещё товаров на {formatPrice(remainingAmount)} для оформления заказа
                </p>
                <Button
                  variant="outline"
                  size="sm"
                  className="mt-3"
                  onClick={() => navigate(`/wholesale/${subdomain}`)}
                >
            ...
            <section className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <Textarea
                  ...
                  placeholder="Пожелания к заказу, вопросы по доставке..."
