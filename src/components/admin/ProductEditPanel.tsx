  ...
  return (
    <div className="bg-gradient-to-b from-muted/40 to-muted/20 border-t border-primary/20 animate-fade-in shadow-inner">
      ...
      <div className="p-2 grid grid-cols-2 sm:grid-cols-3 gap-1.5">
        ...
        {/* Категория - всегда использует catalog-specific categories */}
          ...
          <Popover>
            ...
            <PopoverContent className="w-64 p-2 bg-background z-50" align="start">
              <div className="space-y-1 max-h-48 overflow-y-auto">
                {/* Категории из базы данных магазина */}
                ...
                {/* Предустановленные категории (если нет из базы) */}
                {storeCategories.length === 0 && predefinedCategories.map((category) => (
                  <label
                    key={category}
                    className="flex items-center gap-2 px-2 py-1.5 rounded hover:bg-muted/50 cursor-pointer"
                  >
                ...
                {/* Кастомные категории (не из базы и не предустановленные) */}
                {selectedCategories
                  .filter(c => 
                    !storeCategories.find(sc => sc.name === c) && 
                    !predefinedCategories.includes(c)
                  )
                  .map((category) => (
                    <label
                      key={category}
